...
import {
  ...
  GraduationCap, Pencil, Check, Trash2, FolderPlus, FolderMinus,
} from 'lucide-react';
...
function App() {
  ...
  const [dragOverFolderId, setDragOverFolderId] = useState<string | null>(null); // drop highlight
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', overflow: 'hidden', background: 'var(--bg-primary)' }}>
      ...
      <aside style={{
        ...
        <div style={{ minWidth: 272, display: 'flex', flexDirection: 'column', height: '100%' }}>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        background: isActive ? 'linear-gradient(135deg, var(--accent), rgba(0,122,255,0.85))' : 'transparent',
                        color: isActive ? '#fff' : 'var(--text-primary)', transition: 'all 0.15s', marginBottom: 2,
                      }}
                      onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'var(--sidebar-item-hover)'; }}
                      onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
                    >
                      ...
                      <button onClick={ev => handleDeleteDoc(ev, d.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: isActive ? 'rgba(255,255,255,0.6)' : 'var(--text-tertiary)', padding: 4, borderRadius: 5, display: 'flex', opacity: 0.6 }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.opacity = '1'; }}
          ...
          {/* Bottom: upload without course */}
          <div style={{ padding: '10px 10px 14px', flexShrink: 0, borderTop: '1px solid var(--sidebar-border)' }}>
        ...
        <ErrorBoundary>
          <main style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
            {!activeDoc ? (
              ...
              <Workspace key={activeDoc.id} doc={activeDoc} onSave={handleSaveDoc} />
            )}
          </main>
        </ErrorBoundary>
      </div>
